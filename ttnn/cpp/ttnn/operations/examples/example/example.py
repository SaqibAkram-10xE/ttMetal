...
try:
    ...
    scales_np = np.random.uniform(
        low=0.00048828125, high=0.0625, size=(n_tiles_scales, elements_per_tile_scales)
    ).astype(np.float32)

    print(f"colIdx_data size (ele): {colIdx_np.size}")
    print(f"rowIdx_data size (ele): {rowIdx_np.size}")
    print(f"scales_data size (ele): {scales_np.size}")
    ...
    # Run the TT-NN op
    # ----------------------------
    # output_tt = ttnn.prim.example(rowIdx_tt, codeBook_tt, colIdx_tt, scales_tt)
    ...
    for idx_b in range(len(output_flat)):
        ...
        scales_val = float(scales_flat[idx_b // 16])
        ...
        expected = float(codeBook_np[codebook_index]) * scales_val
